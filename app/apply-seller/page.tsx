...
export default function ApplySellerPage() {
  ...
  const startCamera = async () => {
    try {
      ...
      alert("يرجى السماح بالوصول للكاميرا لالتقاط الصورة.");
    }
  };
  ...
  const uploadImageToStorage = async (base64String: string, fileName: string) => {
    const imageRef = ref(storage, `kyc_documents/${user.uid}/${fileName}_${Date.now()}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setErrorMsg("");
    setUploadProgressMsg("جاري رفع المستندات... يرجى الانتظار.");

    try {
      // 1. Upload Images to Firebase Storage and get URLs
      const idFrontUrl = await uploadImageToStorage(images.idFront, 'idFront');
      const idBackUrl = await uploadImageToStorage(images.idBack, 'idBack');
      const selfieUrl = await uploadImageToStorage(images.selfie, 'selfie');

      setUploadProgressMsg("تم رفع المستندات بنجاح، جاري إرسال الطلب...");

      // 2. Prepare Data with proper image URLs
      ...
      if(!successMsg) setIsSubmitting(false); // keep submitting state true if success to prevent double clicks during redirect
    }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto py-12 px-4 space-y-8" dir="rtl">
      ...
      {uploadProgressMsg && !successMsg && !errorMsg && (
        <div className="bg-blue-50 text-blue-600 p-4 rounded-xl flex items-center gap-3 border border-blue-200">
          <Loader2 className="w-6 h-6 shrink-0 animate-spin" />
          <p className="font-bold">{uploadProgressMsg}</p>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden p-6 md:p-8">
        ...
        {currentStep === 2 && (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <p className="text-sm text-slate-500">أدخل بيانات الدفع الخاصة بك لاستلام أرباحك. (يرجى إدخال وسيلة واحدة على الأقل).</p>
        ...
        {currentStep === 3 && (
          <form onSubmit={handleSubmit} className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <span className="text-sm font-bold text-slate-900 block border-b border-slate-100 pb-2">التقاط صورة سيلفي مباشرة</span>
                ...
                <div className="bg-slate-900 rounded-xl overflow-hidden relative aspect-[3/4] flex flex-col items-center justify-center shadow-inner">
                  ...
                  {images.selfie ? (
                    ...
                    <>
                      <video ref={videoRef} autoPlay playsInline muted className={`w-full h-full object-cover ${!isCameraActive ? 'hidden' : ''}`}></video>
